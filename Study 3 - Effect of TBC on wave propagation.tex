...
Three wave modes with clear peaks in group velocity were targeted, to be excited with a wedge transducer. $B_1$ at 1~MHz (30.1\degree), $B_3$ at 2.4~MHz (25.3\degree), and $B_5$ at 4.4~MHz (25.6\degree). These frequencies correspond to peaks in the energy velocity dispersion curves, where velocity is at a maximum for these modes. The use of wedge transducers further isolates each mode.  

Time-displacement data is transformed into frequency-wavenumber data using 2D-FFT from spatial B-scan data, using 90 point probes equally spaced 0.8 mm apart. This allows individual modes to be identified, and compared with dispersion curves for verification. Both in-plane ($x$) and out-of-plane ($y$) displacement was monitored. 

Temperature dependant material properties of Young's modulus for TBC materials are provided in~\cref{table:TBCmatprops}, and shown in~\cref{fig:TBC_E}. All three sources of values for the top coat (YSZ) show linear reductions in $E$ with temperature, while the reduction in $E$ for the bond coat (MCrAlY) increases with temperature. Saucedo-Mora \textit{et al.}\cite{Saucedo-Mora2015} suggest that the data provided by Beghini \textit{et al.}\cite{Beghini2001} shows a significant decrease in $E$ due to microcracking. The elevated initial values of $E$ in comparison to the other sources suggests that the tested material has undergone thermal aging~\cite{Yamazaki2007,Nath2015}. 

The change in Young's modulus with temperature for the TGO is ignored, as the effect is considered insignificant due to the thickness of the layer.

The Poisson's ratio of the materials is temperature independent, 0.33 for the bond coat, and 0.2 for the top coat. Changes in density due to thermal expansion are not considered in this study, as the effect on wave propagation is small in comparison to changes in Young's modulus, as discussed in~\cref{effectoftemponwaveprop}.
...
\begin{table}[h]
    ...
    Gregori \textit{et al.}\cite{Gregori2007} & Top Coat & 22.00     & 21.05  & 20.14  & 19.26  & 18.43  & 17.62 \\ \bottomrule
    \end{tabular}
    \caption{Temperature dependant material properties of TBC materials.}\label{table:TBCmatprops}
\end{table}
