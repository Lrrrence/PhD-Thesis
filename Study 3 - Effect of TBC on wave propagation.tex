...
As seen in \cref{fig:inconeldispTBCgroup20c,fig:inconeldispTBCgroup1020c} the $B_5$ mode shows a distinct energy velocity peak, that maybe be suitable for preferential excitation. The large shift in the mode (reducing in frequency) from low to high temperature, and the relatively small bandwidth of the peak, makes it difficult to select a suitable excitation frequency that does not fall within a dispersive region. A frequency of 3.5 MHz was used in this study, as it represents the energy velocity maxima of the mode, which ensures that value decreases with increasing temperature. Due to the similarity in phase velocity the $B_4$ mode is also excited, however the group velocity of that mode is substantially slower than that of $B_5$, which separates it in the time domain.
...
Dispersion has been shown to have a large impact on the accuracy of time-of-flight measurement, which causes an over-estimation of the reduction in group velocity with temperature. In comparison to pure Inconel 718 (see \cref{fig:groupshift}), even the lower order modes ($B_0$ \& $B_1$, below the cutoff of $B_2$) do not exhibit areas of relatively low dispersion, and so the effect of dispersion on signal propagation in a NGV with TBC applied is always likely to be high. 
