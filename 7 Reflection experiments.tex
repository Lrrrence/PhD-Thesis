...
The wedges were removed from the plate and the couplant was reapplied five times for each hole size, taking ten measurements per reapplication, for a total of fifty measurements per hole size.

\begin{figure}[ht]
    \centering
    \includegraphics[width=.8\textwidth]{./figures/holeampboxplot.eps}
    \caption{Boxplot showing change in reflection signal amplitude with hole size.}\label{fig:holeampboxplot}
\end{figure}

Results are shown in Figure~\ref{fig:holeampboxplot}. The mean is shown in green. There is a linear relationship (R$^2$~=~0.9994) between increasing hole size and increasing amplitude. The average increase in amplitude for a \diameter~0.5~mm change in hole size is 0.216~V $\pm$ 0.014~V.